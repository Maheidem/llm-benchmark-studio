...
async def export_leaderboard_csv(
    ...
    if type == "tool_eval":
        ...
        writer.writerow([
            "rank", "model", "model_id", "avg_tool_pct", "avg_param_pct",
            "avg_overall_pct", "total_evals", "last_eval",
        ])
        for rank, m in enumerate(models, 1):
            writer.writerow([
                rank, m["model"], m["model_id"], m["avg_tool_pct"],
        ...
        writer.writerow([
            "rank", "model_id", "avg_tps", "avg_ttft_ms",
            "avg_cost", "total_runs", "last_run",
        ])
        for rank, m in enumerate(models, 1):
            writer.writerow([
                rank, m["model_id"], m["avg_tps"],
...
async def export_tool_eval_csv(user: dict = Depends(auth.get_current_user)):
    ...
    writer = csv.writer(output)
    writer.writerow([
        "timestamp", "suite_name", "model_id", "model_display_name",
        "tool_accuracy_pct", "param_accuracy_pct", "overall_score_pct",
        "total_cases", "cases_passed",
    ])
    ...
    for run in runs:
        ...
        for s in summaries:
            writer.writerow([
                ...
                run.get("suite_name", ""),
                s.get("model_id", ""),
                s.get("model_display_name", ""),
                s.get("tool_accuracy_pct", ""),
                s.get("param_accuracy_pct", ""),
                s.get("overall_score_pct", ""),
                s.get("total_cases", ""),
                s.get("cases_passed", ""),
            ])
...
async def export_eval_json(eval_id: str, user: dict = Depends(auth.get_current_user)):
    ...
    export_data = {
        ...
        "results": await db.get_case_results(eval_id),
        "summary": await db.get_case_results_summary(eval_id),
    }
...
async def export_run_csv(run_id: str, user: dict = Depends(auth.get_current_user)):
    ...
    writer = csv.writer(output)
    writer.writerow([
        "model_id", "run_number", "tokens_per_second", "ttft_ms",
