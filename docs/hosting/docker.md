...
LLM Benchmark Studio provides a Docker image for production deployments.
...
The Dockerfile uses a Python 3.13 slim base image:
...
COPY app.py benchmark.py auth.py db.py keyvault.py provider_params.py index.html config.yaml ./
...
- Build arg `APP_VERSION` sets the reported version
